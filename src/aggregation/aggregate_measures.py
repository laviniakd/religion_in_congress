...
#congress_df['classifier_probability_list'] = congress_df.index.to_series().progress_apply(get_prob_religious_list)
#congress_df['classifier_label'] = congress_df.index.to_series().progress_apply(get_religious_label_list)
...
mpnet_scores['congress_idx'] == mpnet_scores['congress_idx'].progress_apply(lambda x: x.replace('tensor(', '').replace(')', '')).apply(int)
...
congress_df['max_cosine_sim_ref'] = congress_df.index.to_series().progress_apply(lambda x: get_max_cs(x, mpnet_scores))
congress_df['verse_citation_ref'] = congress_df.index.to_series().progress_apply(lambda x: get_max_citation(x, mpnet_scores))
congress_df['og_text_ref'] = congress_df.index.to_series().progress_apply(lambda x: get_max_text(x, mpnet_scores))

def get_max_values(index_num, df):
    relevant_rows = df[df['congress_idx'] == index_num]
    if not relevant_rows.empty:
        idx_of_verse = relevant_rows['cosine_similarity'].idxmax()
        cs, citation, text = relevant_rows.loc[idx_of_verse, ['cosine_similarity', 'verse_citation', 'text']]
        return (cs, citation, text)
    else:
        return (-1, '', '')

congress_df['max_cosine_sim_ref'], congress_df['verse_citation_ref'], congress_df['og_text_ref'] = zip(*congress_df.index.map(lambda x: get_max_values(x, mpnet_scores)))
...
#print(f'len of idx_to_ppl_dict: {len(idx_to_ppl_dict)}')
#print(f'len of congress_df: {len(congress_df.index)}')
#print(f'max index in congress_df: {congress_df.index.to_series().max()}')
#print(f'max index in idx_to_ppl_dict: {sorted(idx_to_ppl_dict.keys(), reverse=True)[0]}')

congress_df['bible_ppl'] = congress_df.index.map(lambda x: idx_to_ppl_dict[str(x)]['bible'])
congress_df['base_ppl'] = congress_df.index.map(lambda x: idx_to_ppl_dict[str(x)]['base'])
