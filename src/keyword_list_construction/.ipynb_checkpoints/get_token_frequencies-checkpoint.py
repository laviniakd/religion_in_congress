...
OUTPUT = '/data/laviniad/sermons-ir/token-frequencies/'

COMPUTE_COCA = False
COMPUTE_PRES = False
COMPUTE_SERMONS = False
...
if COMPUTE_COCA:
    ...
    coca_dict = coca_utils.load_COCA_from_raw(COCA_PATH)
    print("Loading DF...")
    coca_df = coca_utils.output_comprehensive_df(coca_dict)
    coca_df['text'] = coca_df['text'].apply(str).apply(word_tokenize)
...
if COMPUTE_PRES:
    ...
    pres_df['text'] = pres_df['text'].apply(word_tokenize)
...
if COMPUTE_CONGRESSIONAL:
    ...
    congress_df = congress_utils.load_full_df_from_raw(CONGRESSIONAL_PATH)
    congress_df['text'] = congress_df['text'].apply(word_tokenize)
...
if COMPUTE_SERMONS:
    ...
    sermon_df['text'] = sermon_df['text'].apply(str).apply(word_tokenize)
