...
# load congressional data
    ...
    filtered_df = load_and_filter_cr(congressional_df, KEYWORD_FILTER_THRESHOLD, EMBED_ALL=args.embed_all, debug=args.debug, sample=args.sample)
...
# load model
    full_model, model = load_embedding_model(args.model_dir, DEVICE)
...
def load_and_filter_cr(congressional_df, KEYWORD_FILTER_THRESHOLD, EMBED_ALL=False, debug=False, sample=-1):

    if debug:
        congressional_df = congressional_df.sample(1000)
        print("In debug mode; sampled 1000 documents from congressional record")
    elif sample > 0:
        if sample < len(congressional_df.index):
            congressional_df = congressional_df.sample(sample)
            print(f"Due to sample being passed, sampled down to {sample} instances")
        else:
...
def load_embedding_model(model_dir, device):
    print("Loading model...")
    full_model = BiModel(device='cpu')
    if model_dir == 'sentence-transformers/all-mpnet-base-v2':
