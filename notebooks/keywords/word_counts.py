...
sns.set(context="notebook", font_scale=3, rc={'figure.figsize':(16,14), 'font.weight': 'normal'}, style='whitegrid')

import data
from data.data_utils import get_lexical_overlap

print(f"File of data module is {data.__file__}")
import collections
...
for d in objs:
    count_dict.update(d[1]) # second thing returned is the token count dict
     
# checks
assert("baptism" not in count_dict.keys())
assert("unclean" not in count_dict.keys())
...
sns.barplot(y=words,x=counts,hue=words,color='#debd00')
plt.xlabel('Count')
...
plt.savefig("/home/laviniad/projects/religion_in_congress/plots/word_counts.pdf", format='pdf', dpi=300, bbox_inches='tight')
print("Saved file to /home/laviniad/projects/religion_in_congress/plots/word_counts.pdf")
